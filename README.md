...
Buat file `publisher.py` yang menghubngkan `paho-mqtt` dengan broker mqtt dengan port 1883 dan keepalive diset 60s serta akan mengirimkan data temperature secara random pada topic `$air/temperature`
...
Sedangkan untuk fungsi `client.loop_forever()` digunakan untuk menjalankan client secara terus menerus untuk _listen_ ke topic tertentu.
...
1. Install package `node-red-contrib-influxdb`
    ...
    1. Click hamburger menu pada kanan atas pada page<br>![click hamburger menu](Images/addInfluxPackage/clickHammenu.jpg)

    2. Click Manage Pallete<br>![click Manage Pallete](Images/addInfluxPackage/clickManagePallete.jpg)

    3. Pilih Tab Install<br>![click Tab Install](Images/addInfluxPackage/clickTabInstall.jpg)

    4. Ketikan `influxdb` pada search bar dan Install package `node-red-contrib-influxdb`<br>![install influx](Images/addInfluxPackage/installInflux.jpg)
...
Jika berhasil maka node `mqtt in` akan tertulis connected
...
4. Select data pada measurement yang sesuai, pada kasus ini `$air/temperature`
    ```sql
...
Jika berhasil maka akan tertampil
